...
const Analytics = () => {
  ...
  const [genreData, setGenreData] = useState<Array<{name: string, value: number}>>([]);
  const [moodData, setMoodData] = useState<Array<{name: string, value: number}>>([]);
  const [energyData, setEnergyData] = useState<Array<{name: string, value: number}>>([]);
  const [uploadTrend, setUploadTrend] = useState<Array<{month: string, uploads: number}>>([]);
  ...
  const fetchAnalytics = async () => {
    ...
    const currentTier = (subscription?.plan_name as TierName) || "Free";
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              {/* Genre Distribution - Available to all */}
              {genreData.length > 0 && (
              ...
              {/* Mood Distribution - Pro Feature */}
              {moodData.length > 0 && (
                <Card className="p-6 card-urban relative">
                  <div className="flex items-center justify-between mb-4">
                  ...
                  {userTier === "Pro" ? (
                    ...
                    <div className="h-[300px] flex items-center justify-center bg-muted/20 rounded-lg border-2 border-dashed border-border">
